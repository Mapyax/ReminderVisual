...
void PopUp::paintEvent(QPaintEvent *event)
{
     ...
     * который является прямоугольником с фиолетовым фоном
     * */
    ...
    // Кисть настраиваем на фиолетовый цвет в режиме полупрозрачности 180 из 255
    painter.setBrush(QBrush(QColor(0,0,120,180)));
    painter.setPen(Qt::NoPen); // Край уведомления не будет выделен

    // Отрисовываем фон с закруглением краёв в 30px
    painter.drawRoundedRect(roundedRect, 30, 30);
}
...
void PopUp::show()
{
    ...
    animation.start();
    timer->start(6000);             // А также стартуем таймер, который запустит скрытие уведомления через 3 секунды
}
